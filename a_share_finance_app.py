...
async function loadStock(code,force=false){showDashboard();$('#results').classList.add('hidden');status(force?'正在重新拉取同花顺数据…':'正在读取本地缓存或拉取数据…');try{current=await api('/api/financials/'+code+(force?'?refresh=1':''));render();loadProfile(code,false);refreshLists();status(current.warning|| (current.from_cache?'已从本地SQLite缓存读取。':'已从同花顺拉取并保存到本地SQLite。'),Boolean(current.warning))}catch(e){status(e.message,true)}}
function showDashboard(){$('#dashboardView').classList.remove('hidden');$('#rankingView').classList.add('hidden');$('#dashboardNav').classList.add('active');$('#rankingNav').classList.remove('active')}
async function showRankings(){$('#dashboardView').classList.add('hidden');$('#rankingView').classList.remove('hidden');$('#dashboardNav').classList.remove('active');$('#rankingNav').classList.add('active');await loadRankings()}
async function loadRankings(){const table=$('#rankingTable');table.innerHTML='<tbody><tr><td class="empty">正在计算全部收藏评分…</td></tr></tbody>';try{const data=await api('/api/favorite-rankings?horizon='+rankingHorizon);rankingRows=data.rankings||[];$('#rankingMethod').innerHTML=`<strong>${esc(data.horizon_label||rankingHorizon)}评分方法</strong><br>${esc(data.methodology||'')}<br><span>覆盖率表示该股票可参与计算的指标权重占比；最终分数已按覆盖率轻度折减。</span>`;renderRankings()}catch(e){table.innerHTML=`<tbody><tr><td class="empty">${esc(e.message)}</td></tr></tbody>`;$('#rankingMethod').textContent='评分加载失败。'}}
function rankFmt(v,type){if(v==null||!Number.isFinite(Number(v)))return'—';if(type==='pct')return(Number(v)*100).toFixed(1)+'%';if(type==='num')return Number(v).toFixed(2);return String(v)}
function renderRankings(){const q=$('#rankSearch').value.trim().toLowerCase();let rows=rankingRows.filter(x=>!q||String(x.code).includes(q)||String(x.name||'').toLowerCase().includes(q));const {key,dir}=rankingSort;rows=rows.slice().sort((a,b)=>{let av=a[key],bv=b[key];if(key==='name')return dir*String(av||'').localeCompare(String(bv||''),'zh-CN');av=av==null?(dir>0?Infinity:-Infinity):Number(av);bv=bv==null?(dir>0?Infinity:-Infinity):Number(bv);return dir*(av-bv)});const cols=[['rank','排名'],['name','股票'],['score','综合评分'],['coverage','覆盖率'],['growth','增长'],['quality','质量'],['cashflow','现金流'],['valuation','估值/回报'],['risk_tags','风险标签']];const arrow=k=>rankingSort.key===k?(rankingSort.dir>0?' ▲':' ▼'):'';let h='<thead><tr>'+cols.map(c=>`<th data-sort="${c[0]}">${c[1]}${arrow(c[0])}</th>`).join('')+'</tr></thead><tbody>';h+=rows.map(x=>`<tr class="${x.rank<=3?'top'+x.rank:''}" onclick="loadStock('${x.code}')"><td class="rank-no">${x.rank}</td><td><strong>${esc(x.name||x.code)}</strong><br><small style="color:var(--muted)">${x.code}</small></td><td class="score">${rankFmt(x.score,'num')}</td><td><span class="coverage"><span class="coverage-bar"><i style="width:${Math.round((x.coverage||0)*100)}%"></i></span>${rankFmt(x.coverage,'pct')}</span></td><td>${rankFmt(x.growth,'pct')}</td><td>${rankFmt(x.quality,'pct')}</td><td>${rankFmt(x.cashflow,'pct')}</td><td>${rankFmt(x.valuation,'pct')}</td><td><span class="risk-tags">${(x.risk_tags||[]).length?(x.risk_tags||[]).map(t=>`<span class="risk-tag">${esc(t)}</span>`).join(''):'<span class="risk-tag ok">未触发</span>'}</span></td></tr>`).join('');if(!rows.length)h+='<tr><td colspan="9" class="empty">暂无匹配的收藏股票</td></tr>';$('#rankingTable').innerHTML=h+'</tbody>';$('#rankSummary').textContent=`显示 ${rows.length} / ${rankingRows.length} 只收藏股`;$('#rankingTable').querySelectorAll('th[data-sort]').forEach(th=>th.onclick=()=>{const k=th.dataset.sort;if(k==='risk_tags')return;if(rankingSort.key===k)rankingSort.dir*=-1;else rankingSort={key:k,dir:k==='rank'||k==='name'?1:-1};renderRankings()})}
...
$('#refresh').onclick=()=>current&&loadStock(current.code,true);$('#exportBtn').onclick=()=>{if(!current){status('请先选择股票。',true);return}status('正在生成Excel，请稍候…');window.location.href='/api/export/'+current.code;setTimeout(()=>status('Excel已生成并开始下载。'),1200)};$('#favoriteBtn').onclick=async()=>{if(!current)return;try{await api('/api/favorites/'+current.code,{method:current.is_favorite?'DELETE':'POST'});current.is_favorite=!current.is_favorite;render();refreshLists()}catch(e){status(e.message,true)}};$('#rankSearch').addEventListener('input',renderRankings);$('#horizonTabs').querySelectorAll('[data-horizon]').forEach(btn=>btn.onclick=()=>{rankingHorizon=btn.dataset.horizon;rankingSort={key:'rank',dir:1};$('#horizonTabs').querySelectorAll('[data-horizon]').forEach(x=>x.classList.toggle('active',x===btn));loadRankings()});refreshLists();
</script></body></html>'''
...
def _rank_values(payload, key, annual=False, count=None):
    ...
    for index, value in enumerate(values):
        ...
        if annual and not (isinstance(year, str) and re.fullmatch(r"\d{4}", year)):
            continue
...
def _rank_latest(payload, key):
    values = _rank_values(payload, key)
    return values[-1] if values else None


def _rank_clip(value, low, high):
    value = _finite(value)
    return None if value is None else max(low, min(high, value))


def _rank_mean(values):
    return statistics.mean(values) if values else None


def _rank_median(values):
    return statistics.median(values) if values else None


def _rank_cagr(values, periods):
    block = values[-min(periods, len(values)):] if values else []
    if len(block) < 2 or block[0] <= 0 or block[-1] <= 0:
        return None
    try:
        result = (block[-1] / block[0]) ** (1 / (len(block) - 1)) - 1
        return result if math.isfinite(result) else None
    except (ValueError, ZeroDivisionError, OverflowError):
        return None


def _positive_share(values):
    return sum(value > 0 for value in values) / len(values) if values else None


def _percentile(values, value, reverse=False):
    sample = sorted(v for v in (_finite(x) for x in values) if v is not None)
    value = _finite(value)
    if value is None or not sample:
        return None
    position = sum(v < value for v in sample) + 0.5 * sum(v == value for v in sample)
    percentile = position / len(sample)
    return 1 - percentile if reverse else percentile


def favorite_rankings(horizon):
    """基于 score_favorites.py 验证模型，动态计算收藏池相对排名。"""
    if horizon not in ("1y", "5y", "10y"):
        raise ValueError("horizon 必须是 1y、5y 或 10y")
    with DB_LOCK, db_conn() as con:
        rows = con.execute("SELECT f.code,COALESCE(NULLIF(s.name,''),f.code) name,s.payload FROM favorites f LEFT JOIN stocks s ON s.code=f.code ORDER BY f.created_at").fetchall()
    records = []
    for row in rows:
        try:
            ...
            payload = {}
        forecast = payload.get("price_forecast") if isinstance(payload.get("price_forecast"), dict) else {}
        composite = forecast.get("composite") if isinstance(forecast.get("composite"), list) else []
        base = _finite(forecast.get("latest_actual_price")) or _finite(payload.get("latest_price"))
        ups = []
        for item in composite:
            median = _finite(item.get("median")) if isinstance(item, dict) else None
            ups.append(median / base - 1 if median is not None and base and base > 0 else None)
        fcf_now = _rank_latest(payload, "free_cash_flow")
        record = {
            "code": row["code"], "name": row["name"] or row["code"],
            "rev_now": _rank_latest(payload, "revenue_growth"), "np_now": _rank_latest(payload, "net_profit_growth"),
            "roe_now": _rank_latest(payload, "roe"), "roic_now": _rank_clip(_rank_latest(payload, "roic"), -.2, .6),
            "ocf_now": _rank_clip(_rank_latest(payload, "ocf_to_profit"), -1, 2), "fcf_now": fcf_now,
            "pe": _rank_clip(payload.get("dynamic_pe"), 0, 120) if (_finite(payload.get("dynamic_pe")) or 0) > 0 else None,
            "div": _rank_clip(_rank_latest(payload, "dividend_yield"), 0, .12),
            "debt": _rank_clip(_rank_latest(payload, "interest_debt_ratio"), 0, 1),
            "cov": _rank_clip(_rank_latest(payload, "interest_coverage"), 0, 20),
            "rev5": _rank_cagr(_rank_values(payload, "revenue", True), 6), "np5": _rank_cagr(_rank_values(payload, "net_profit", True), 6),
            "rev10": _rank_cagr(_rank_values(payload, "revenue", True), 11), "np10": _rank_cagr(_rank_values(payload, "net_profit", True), 11),
            "roe5": _rank_mean(_rank_values(payload, "roe", True, 5)), "roic5": _rank_clip(_rank_mean(_rank_values(payload, "roic", True, 5)), -.2, .6),
            "roe10": _rank_mean(_rank_values(payload, "roe", True, 10)), "roic10": _rank_clip(_rank_mean(_rank_values(payload, "roic", True, 10)), -.2, .6),
            "ocf5": _rank_clip(_rank_median(_rank_values(payload, "ocf_to_profit", True, 5)), -1, 2), "fcf5": _positive_share(_rank_values(payload, "free_cash_flow", True, 5)),
            "ocf10": _rank_clip(_rank_median(_rank_values(payload, "ocf_to_profit", True, 10)), -1, 2), "fcf10": _positive_share(_rank_values(payload, "free_cash_flow", True, 10)),
            "tr": _rank_clip(payload.get("total_return_cagr"), -.3, .6),
            "up1": _rank_clip(ups[0] if ups else None, -.5, 1), "up5": _rank_clip(ups[4] if len(ups) > 4 else None, -.5, 1),
            "forecast_disp": None,
        }
        if composite and base and isinstance(composite[0], dict):
            low, high = _finite(composite[0].get("low")), _finite(composite[0].get("high"))
            record["forecast_disp"] = _rank_clip((high - low) / base if low is not None and high is not None else None, 0, 3)
        records.append(record)
    weights = {
        "1y": {"rev_now": .15, "np_now": .20, "roe_now": .075, "roic_now": .075, "ocf_now": .10, "fcf_now": .05, "pe": .10, "div": .05, "debt": .05, "cov": .05, "up1": .10},
        "5y": {"rev5": .15, "np5": .20, "roe5": .15, "roic5": .15, "ocf5": .08, "fcf5": .07, "debt": .05, "cov": .05, "pe": .05, "div": .02, "up5": .03},
        "10y": {"rev10": .15, "np10": .20, "roe10": .15, "roic10": .20, "ocf10": .10, "fcf10": .10, "debt": .04, "cov": .03, "div": .01, "tr": .02},
    }
    groups = {
        "1y": {"growth": ("rev_now", "np_now"), "quality": ("roe_now", "roic_now"), "cashflow": ("ocf_now", "fcf_now"), "valuation": ("pe", "div", "debt", "cov", "up1")},
        "5y": {"growth": ("rev5", "np5"), "quality": ("roe5", "roic5"), "cashflow": ("ocf5", "fcf5"), "valuation": ("pe", "div", "debt", "cov", "up5")},
        "10y": {"growth": ("rev10", "np10"), "quality": ("roe10", "roic10"), "cashflow": ("ocf10", "fcf10"), "valuation": ("debt", "cov", "div", "tr")},
    }
    reverse = {"pe", "debt", "forecast_disp"}
    weight = weights[horizon]
    for record in records:
        numerator = denominator = coverage = 0.0
        percentiles = {}
        for key, item_weight in weight.items():
            percentile = _percentile([candidate.get(key) for candidate in records], record.get(key), key in reverse)
            percentiles[key] = percentile
            if percentile is not None:
                numerator += item_weight * percentile; denominator += item_weight; coverage += item_weight
        score = 100 * numerator / denominator if denominator else 0.0
        tags = []
        if horizon == "1y":
            if record["np_now"] is not None and record["np_now"] < 0: score -= min(15, abs(record["np_now"]) * 20); tags.append("利润负增长")
            if record["fcf_now"] is not None and record["fcf_now"] < 0: score -= 5; tags.append("自由现金流为负")
            if record["ocf_now"] is not None and record["ocf_now"] < .5: score -= 5; tags.append("现金含量偏低")
            if record["pe"] is not None and record["pe"] > 60: score -= 5; tags.append("高估值")
            if record["forecast_disp"] is not None and record["forecast_disp"] > 1: score -= 3; tags.append("预测分歧大")
        else:
            growth_key, fcf_key = ("np5", "fcf5") if horizon == "5y" else ("np10", "fcf10")
            if record[growth_key] is not None and record[growth_key] < 0: score -= 8; tags.append("长期利润收缩")
            if record[fcf_key] is not None and record[fcf_key] < .5: score -= 6; tags.append("现金流稳定性低")
        if coverage < .5: tags.append("数据覆盖偏低")
        record["score"] = round(max(0, min(100, score * (.85 + .15 * coverage))), 2)
        record["coverage"] = round(coverage, 4)
        record["risk_tags"] = tags
        for group, keys in groups[horizon].items():
            values = [percentiles.get(key) for key in keys if percentiles.get(key) is not None]
            record[group] = round(statistics.mean(values), 4) if values else None
    records.sort(key=lambda item: (-item["score"], -item["coverage"], item["code"]))
    result = []
    display_labels = {"1y": "近1年", "5y": "近5年", "10y": "近10年"}
    for rank, record in enumerate(records, 1):
        result.append({key: record.get(key) for key in ("code", "name", "score", "coverage", "risk_tags", "growth", "quality", "cashflow", "valuation")} | {"rank": rank})
    methodology = "各指标在当前全部收藏股中计算百分位，增长、资本回报、现金流、负债/偿息、估值及预测回报按期限加权；缺失指标不计入分母，覆盖率对最终得分作15%的轻度折减，并应用透明风险扣分。银行等行业缺失毛利率、合同负债等字段不会按零分处理。"
    return {"horizon": horizon, "horizon_label": display_labels[horizon], "count": len(result), "rankings": result, "display_metrics": {"growth": "增长百分位", "quality": "资本质量百分位", "cashflow": "现金流百分位", "valuation": "估值/回报百分位"}, "methodology": methodology}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif path == "/api/favorite-rankings":
                horizon = urllib.parse.parse_qs(p.query).get("horizon", ["1y"])[0]
                self.send_json(favorite_rankings(horizon))
