...
function renderForecast(){
 ...
 box.innerHTML=`<div class="toolbar"><strong>未来5年股价预测</strong><span class="legend">${legend}</span></div><svg id="forecastSvg" viewBox="0 0 1000 340" preserveAspectRatio="none" style="width:100%;height:285px;display:block"></svg><div class="forecast-summary">${esc(fc.method)}<br>基准EPS ${fc.current_eps}元 · 近5年净利润CAGR ${fc.base_cagr}% · 最新实际不复权股价 ${f2(actual)}元<br><span style="color:var(--red)">${esc(fc.disclaimer)}</span></div><div class="holder-wrap"><table class="forecast-table"><thead><tr><th>年度</th>${methodKeys.map(k=>`<th>${methods[k]?.name||k}</th>`).join('')}<th>综合区间</th><th>综合中位</th></tr></thead><tbody>${rows}</tbody></table></div><div class="forecast-summary" style="margin-top:8px">${methodKeys.map(k=>`${methods[k]?.name||k}：${esc(methods[k]?.assumption||'')}`).join('；')}</div>`;
